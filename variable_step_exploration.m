%% variable integration graphs

% Run your adaptive step size integrator multiple times to simulate an orbiting planet, 
% across a range of desired error values. Make sure to record the global truncation error, 
% the average step size, the number of function evaluations, and the step failure rate.
...
tspan = [0, 8]; h_ref = 0.05;
err_des = [0.05, 0.3];

for i = 1:length(err_des)

    [t_list, X_list, h_avg, total_evals, step_fail_rate] = explicit_RK_variable_step_integration ...
        (my_rate, tspan, V0, h_ref, BT, p, err_des(i));

    X_final_exact = compute_planetary_motion(tspan(2), V0, orbit_params);
    g_error = norm(X_final_exact - X_list(end, :));
